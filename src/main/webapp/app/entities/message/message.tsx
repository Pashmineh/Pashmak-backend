...
export class Message extends React.Component<IMessageProps, IMessageState> {
  ...
  render() {
    const { messageList, match, totalItems } = this.props;
    return (
      <div>
        <h2 id="message-heading">
          Messages
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp; Create new Message
          </Link>
        </h2>
          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={this.sort('sendTime')}>
                  Send Time <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={this.sort('messageType')}>
                  Message Type <FontAwesomeIcon icon="sort" />
                </th>
            ...
            <tbody>
              {messageList.map((message, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <TextFormat type="date" value={message.sendTime} format={APP_DATE_FORMAT} />
                  </td>
                  <td>{message.messageType}</td>
                  <td>{message.message}</td>
                  <td>{message.userLogin ? message.userLogin : ''}</td>
